...
"""
    SPKSegmentCache9(head::SPKSegmentHeader9)
...
function SPKSegmentType9(daf::DAF, desc::DAFSegmentDescriptor)
    ...
    caches = build_thread_cache(() -> SPKSegmentCache9(header))
...
function spk_vector6(daf::DAF, seg::SPKSegmentType9, time::Number)
    ...
    if head.type == 9
        ...
        x, vx = hermite_derivative1(data.buff, data.states, data.epochs, time, 1, head.N)
        y, vy = hermite_derivative1(data.buff, data.states, data.epochs, time, 2, head.N)
        z, vz = hermite_derivative1(data.buff, data.states, data.epochs, time, 3, head.N)
    end
...
function spk_vector9(daf::DAF, seg::SPKSegmentType9, time::Number)
    ...
    if head.type == 9
        ...
        vx, ax = lagrange_derivative1(data.buff, data.states, data.epochs, time, 4, head.N)
        vy, ay = lagrange_derivative1(data.buff, data.states, data.epochs, time, 5, head.N)
        vz, az = lagrange_derivative1(data.buff, data.states, data.epochs, time, 6, head.N)
    else
        # Interpolate the position, velocity and acceleration
        x, vx, ax = hermite_derivative2(data.buff, data.states, data.epochs, time, 1, head.N)
        y, vy, ay = hermite_derivative2(data.buff, data.states, data.epochs, time, 2, head.N)
        z, vz, az = hermite_derivative2(data.buff, data.states, data.epochs, time, 3, head.N)
    end
...
function spk_vector12(daf::DAF, seg::SPKSegmentType9, time::Number)
    ...
    if head.type == 9
        ...
        vx, ax, jx = lagrange_derivative2(data.buff, data.states, data.epochs, time, 4, head.N)
        vy, ay, jy = lagrange_derivative2(data.buff, data.states, data.epochs, time, 5, head.N)
        vz, az, jz = lagrange_derivative2(data.buff, data.states, data.epochs, time, 6, head.N)
    else
        # Interpolate the position, velocity, acceleration and jerk
        x, vx, ax, jx = hermite_derivative3(data.buff, data.states, data.epochs, time, 1, head.N)
        y, vy, ay, jy = hermite_derivative3(data.buff, data.states, data.epochs, time, 2, head.N)
        z, vz, az, jz = hermite_derivative3(data.buff, data.states, data.epochs, time, 3, head.N)
    end
...
function get_coefficients!(daf::DAF, head::SPKSegmentHeader9, cache::SPKSegmentCache9,
    ...
    # Load the state coefficients, which are row-major in the DAF record.
