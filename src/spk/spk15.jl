...
function spk_vector3(::DAF, seg::SPKSegmentType15, time::Number)
    ...
    pos, _ = propagate_twobody(data, time_offset)
    if head.vj2

        # Account for the J2 effect. The rotations can be performed in any order, but if
...
function spk_vector6(::DAF, seg::SPKSegmentType15, time::Number)
    ...
    pos, vel = propagate_twobody(data, time_offset)
    if head.vj2
        ...
        if head.j2f != 1
            pos = vrot(pos, head.tp, periapsis_delta)
            vel = vrot(vel, head.tp, periapsis_delta)
        end

        # Regresses the line of nodes by rotating the state about the pole of the body
        if head.j2f != 2
            pos = vrot(pos, head.pv, node_delta)
            vel = vrot(vel, head.pv, node_delta)
        end
...
function compute_j2_effects(head::SPKSegmentHeader15, pos, time_offset::Number)

    # Compute the change in mean anomaly since periapsis
    mean_anomaly_delta = head.dmdt*time_offset

    # Compute the angle wrapped_anomaly such that -pi < wrapped_anomaly < pi and mean_anomaly_delta = wrapped_anomaly + 2kpi
    wrapped_anomaly = mod(mean_anomaly_delta, 2pi)
    if abs(wrapped_anomaly) > pi
        wrapped_anomaly -= 2pi*sign(wrapped_anomaly)
    end

    # Get the accumulated true anomaly from the propagated state wrapped_anomaly and the accumulated
    # mean anomaly prior to this orbit
    TA = vsep(head.pa, pos)*sign(wrapped_anomaly) + mean_anomaly_delta - wrapped_anomaly

    # Determine how far the line of nodes and periapsis have moved
    node_delta = TA*head.kn
    periapsis_delta = TA*head.kp

    return node_delta, periapsis_delta
