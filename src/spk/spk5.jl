...
function SPKSegmentType5(daf::DAF, desc::DAFSegmentDescriptor)
    ...
    caches = build_thread_cache(() -> SPKSegmentCache5(header))
...
function spk_vector3(daf::DAF, seg::SPKSegmentType5, time::Number)
    ...
    time_offset1 = time - t1
    time_offset2 = time - t2

    # Propagate the two states
    p1, _ = propagate_twobody(data.c1, time_offset1)
    p2, _ = propagate_twobody(data.c2, time_offset2)
...
function spk_vector6(daf::DAF, seg::SPKSegmentType5, time::Number)
    ...
    # Propagate the two states
    p1, v1 = propagate_twobody(data.c1, time_offset1)
    p2, v2 = propagate_twobody(data.c2, time_offset2)
