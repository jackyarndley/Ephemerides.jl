...
function SPKSegmentType2(daf::DAF, desc::DAFSegmentDescriptor)
    ...
    caches = build_thread_cache(() -> SPKSegmentCache2(header))
...
function spk_vector3(daf::DAF, seg::SPKSegmentType2, time::Number)
    ...
    x, y, z = chebyshev(data.A, t, 0, head.N)
...
function spk_vector6(daf::DAF, seg::SPKSegmentType2, time::Number)
    ...
    @inbounds if head.type == 2
        x, y, z, vx, vy, vz = chebyshev_derivative1(data.A, t, 0, head.N, data.p[3])
    else
        x, y, z = chebyshev(data.A, t, 0, head.N)
        vx, vy, vz = chebyshev(data.A, t, 3, head.N)
    end
...
function spk_vector9(daf::DAF, seg::SPKSegmentType2, time::Number)
    ...
    @inbounds if head.type == 2
        x, y, z, vx, vy, vz, ax, ay, az = chebyshev_derivative2(
            data.A, t, 0, head.N, data.p[3]
        )

    else
        x, y, z = chebyshev(data.A, t, 0, head.N)
        vx, vy, vz, ax, ay, az = chebyshev_derivative1(data.A, t, 3, head.N, data.p[3])
    end
...
function spk_vector12(daf::DAF, seg::SPKSegmentType2, time::Number)
    ...
    @inbounds if head.type == 2
        x, y, z, vx, vy, vz, ax, ay, az, jx, jy, jz = chebyshev_derivative3(
            data.A, t, 0, head.N, data.p[3]
        )
...
function get_coefficients!(daf::DAF, head::SPKSegmentHeader2, cache::SPKSegmentCache2,
    ...
    read_doubles_rowmajor!(
        cache.A, array(daf), k, head.ncomp, head.N, endian(daf)
    )
...
@inline function normalise_time(cache::SPKSegmentCache2, time::Number)
    @inbounds return (time - cache.p[1])*cache.p[3]
end
