...
function spk_vector6(daf::DAF, head::SPKSegmentHeader18, data::SPKSegmentCache18, t::Number)
    ...
    elseif head.subtype == 1
        ...
        x, vx = hermite_derivative1(data.buff, data.states, data.epochs, t, 1, N)
        y, vy = hermite_derivative1(data.buff, data.states, data.epochs, t, 2, N)
        z, vz = hermite_derivative1(data.buff, data.states, data.epochs, t, 3, N)
...
function spk_vector9(daf::DAF, head::SPKSegmentHeader18, data::SPKSegmentCache18, t::Number)
    ...
    if head.subtype == 0
        ...
        vx, ax = hermite_derivative1(data.buff, data.states, data.epochs, t, 7, N)
        vy, ay = hermite_derivative1(data.buff, data.states, data.epochs, t, 8, N)
        vz, az = hermite_derivative1(data.buff, data.states, data.epochs, t, 9, N)
    ...
    elseif head.subtype == 1
        ...
        # Interpolate the velocity and acceleration
        vx, ax = lagrange_derivative1(data.buff, data.states, data.epochs, t, 4, N)
        vy, ay = lagrange_derivative1(data.buff, data.states, data.epochs, t, 5, N)
        vz, az = lagrange_derivative1(data.buff, data.states, data.epochs, t, 6, N)

    else
        # SPK type 19 mini-segment
        # Interpolate the position, velocity and acceleration
        x, vx, ax = hermite_derivative2(data.buff, data.states, data.epochs, t, 1, N)
        y, vy, ay = hermite_derivative2(data.buff, data.states, data.epochs, t, 2, N)
        z, vz, az = hermite_derivative2(data.buff, data.states, data.epochs, t, 3, N)
...
function spk_vector12(daf::DAF, head::SPKSegmentHeader18, data::SPKSegmentCache18, t::Number)
    ...
    if head.subtype == 0
        ...
        vx, ax, jx = hermite_derivative2(data.buff, data.states, data.epochs, t, 7, N)
        vy, ay, jy = hermite_derivative2(data.buff, data.states, data.epochs, t, 8, N)
        vz, az, jz = hermite_derivative2(data.buff, data.states, data.epochs, t, 9, N)
    ...
    elseif head.subtype == 1
        ...
        # Interpolate the velocity, acceleration and jerk
        vx, ax, jx = lagrange_derivative2(data.buff, data.states, data.epochs, t, 4, N)
        vy, ay, jy = lagrange_derivative2(data.buff, data.states, data.epochs, t, 5, N)
        vz, az, jz = lagrange_derivative2(data.buff, data.states, data.epochs, t, 6, N)

    else
        # SPK type 19 mini-segment
        # Interpolate the position, velocity, acceleration and jerk
        x, vx, ax, jx = hermite_derivative3(data.buff, data.states, data.epochs, t, 1, N)
        y, vy, ay, jy = hermite_derivative3(data.buff, data.states, data.epochs, t, 2, N)
        z, vz, az, jz = hermite_derivative3(data.buff, data.states, data.epochs, t, 3, N)
